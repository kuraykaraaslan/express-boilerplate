...
import redisInstance from '../../shared/libs/redis';
...
export default class MailService {
    ...
    static readonly TEMPLATE_PATH = path.join(pwd, 'src', 'mail', 'views');
    ...
    static async sendWelcomeEmail(user: User | SafeUser) {
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'welcome.ejs'), {
    ...
    static async sendNewLoginEmail(user: User | SafeUser, userSession?: SafeUserSession) {
        ...
        //console path
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'new-login.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
            device: "Unknown",
            ip: "Unknown",
    ...
    static async sendForgotPasswordEmail(
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'forgot-password.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
    ...
    static async sendPasswordResetSuccessEmail(
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'password-reset.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
            loginLink: MailService.FRONTEND_LOGIN_LINK,
            supportEmail: MailService.FRONTEND_SUPPORT_EMAIL,
    ...
    static async sendOTPEmail({
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'otp.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
            loginLink: MailService.FRONTEND_LOGIN_LINK,
    ...
    static async sendOTPEnabledEmail(email: string, name?: string) {
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'otp-enabled.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
            loginLink: MailService.FRONTEND_LOGIN_LINK,
            resetPasswordLink: MailService.FRONTEND_RESET_PASSWORD_LINK,
    ...
    static async sendOTPDisabledEmail(email: string, name?: string) {
        ...
        const emailContent = await ejs.renderFile(path.join(this.TEMPLATE_PATH, 'otp-disabled.ejs'), {
            user: { name: name || email },
            appName: MailService.APPLICATION_NAME,
            loginLink: MailService.FRONTEND_LOGIN_LINK,
            resetPasswordLink: MailService.FRONTEND_RESET_PASSWORD_LINK,
